...
function dateToTimestamp(date) {
  return new Date(date) - new Date('00:00 01.01.1970 UTC');
}
...
function getNextFriday(date) {
  ...
  return new Date(+date + (((11 - numberDay) % 7) + 1) * 24 * 60 * 60 * 1000);
}
...
function getCountDaysInMonth(month, year) {
  ...
  if (
    (new Date(year + 1, 0) - new Date(year, 0)) / (1000 * 60 * 60 * 24) ===
    366
  )
    dayCounts[1] = 29;
...
function formatDate(date) {
  ...
  const ampm =
    hours > 12 || (hours === 12 && (minutes > 0 || seconds > 0)) ? 'PM' : 'AM';
...
function getCountWeekendsInMonth(month, year) {
  ...
  const dayCount = (new Date(year, month) - date) / (1000 * 60 * 60 * 24);
  let weekendCount = 0;
  for (let i = 0; i < dayCount; i += 1) {
    const newDate = new Date(+date + 1000 * 60 * 60 * 24 * i);
    const dayIndex = newDate.getDay();
    if (dayIndex === 6 || dayIndex === 0) weekendCount += 1;
  }
  return weekendCount;
}
...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil((date - startOfYear + 1) / (1000 * 60 * 60 * 24 * 7));
}
...
function getNextFridayThe13th(date) {
  ...
  const shiftDay = 1000 * 60 * 60 * 24;
  let res = new Date(+date + ((12 - day) % 7) * shiftDay);
  while (res.getDate() !== 13) {
    res = new Date(+res + 7 * shiftDay);
  }
  return res;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  let date = new Date(yearStart, monthStart - 1, dayStart);
  ...
  while (date <= dateEnd) {
    ...
    workingIndex += 1;
    if (workingIndex === countWorkDays) {
      date = new Date(+date + shiftDay * (countOffDays + 1));
      workingIndex = 0;
    } else {
      date = new Date(+date + shiftDay);
    }
  }
  return res;
}
...
function isLeapYear(date) {
  const year = date.getFullYear();
  const differentOfYears = new Date(year + 1, 0) - new Date(year, 0);
  const daysCount = differentOfYears / (1000 * 60 * 60 * 24);
  return daysCount === 366;
}
